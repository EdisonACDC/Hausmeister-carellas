...
CURRENT_INGRESS_IS_ADMIN = ContextVar('current_ingress_is_admin', default=True)
...
def qr_file_response(url: str, filename: str, download: int = 0, pdf: int = 0, size: int = 10, brother: int = 0, tape: int = 24):
    ...
    qr_size_cm = max(5, min(15, int(size or 10)))
    dpi = 300
    page_width = round(21 / 2.54 * dpi)
    page_height = round(29.7 / 2.54 * dpi)
    qr_pixels = round(qr_size_cm / 2.54 * dpi)
    source = qr.get_image() if hasattr(qr, 'get_image') else qr
    resized = source.convert('RGB').resize((qr_pixels, qr_pixels), Image.Resampling.NEAREST)
    page_image = Image.new('RGB', (page_width, page_height), 'white')
    page_image.paste(resized, ((page_width - qr_pixels) // 2, (page_height - qr_pixels) // 2))
    buffer = io.BytesIO()
    page_image.save(buffer, format='PDF', resolution=dpi)
    buffer.seek(0)
    pdf_filename = f'{Path(filename).stem}-{qr_size_cm}cm.pdf'
    disposition = 'attachment' if download else 'inline'
...
def page(title: str, body: str, public: bool = False, lang: str = 'it', back_url: str = '', close_on_back: bool = False, manager: bool = False):
    ...
    settings_link = '<a class="side-link" href="settings" onclick="return adminGo(\'settings\')">⚙ Impostazioni</a>' if CURRENT_INGRESS_IS_ADMIN.get() else ''
    access_label = 'Add-on in esecuzione' if CURRENT_INGRESS_IS_ADMIN.get() else 'Accesso titolare'
...
async def ingress_role_guard(request: Request, call_next):
    ...
    if path == '/settings' or path.startswith('/settings/') or path == '/pin' or path.startswith('/pin/'):
        return ingress_access_page(
            'Area riservata all’amministratore',
            'Le impostazioni, le password, gli utenti e le configurazioni tecniche possono essere gestiti soltanto da un amministratore.',
            403,
        )
    # I vecchi collegamenti della precedente interfaccia in sola lettura vengono
    # ricondotti alle corrispondenti pagine operative senza interrompere l'utente.
    if path == '/owner':
        request.scope['path'] = '/'
        request.scope['raw_path'] = b'/'
    elif path.startswith('/owner/'):
        new_path = path.removeprefix('/owner')
        request.scope['path'] = new_path
        request.scope['raw_path'] = new_path.encode()
    context_token = CURRENT_INGRESS_IS_ADMIN.set(False)
    try:
        return await call_next(request)
    finally:
...
def settings_page(message: str = ''):
    ...
    group_qr_actions = qr_action_buttons('settings/group-qr', 'group', 'qr-tutte-le-zone.png') if group_pin else ''
...
def manager_zone_detail(request: Request, zone_id: int):
    ...
    qr_actions = qr_action_buttons(f'/manager/zone/{zone_id}/qr', f'manager-zone-{zone_id}', f'zona-{zone_id}.png', lang)
