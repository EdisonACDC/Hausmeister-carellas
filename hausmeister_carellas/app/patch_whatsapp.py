...
text = text.replace("(('Elettrico', 'Elektrik'), ('Idraulico', 'Sanitär / Wasser'), ('Climatizzazione', 'Klimaanlage'), ('Porta/Finestra', 'Tür / Fenster'), ('Attrezzatura cucina', 'Küchengerät'), ('Altro', 'Sonstiges'))", "(('Elettrico', 'Elektrik'), ('Idraulico', 'Sanitär / Wasser'), ('Muratore', 'Maurer / Bau'), ('Climatizzazione', 'Klimaanlage'), ('Porta/Finestra', 'Tür / Fenster'), ('Attrezzatura cucina', 'Küchengerät'), ('Altro', 'Sonstiges'))")
text = text.replace("(('Elettrico', 'Elettrico'), ('Idraulico', 'Idraulico'), ('Climatizzazione', 'Climatizzazione'), ('Porta/Finestra', 'Porta / Finestra'), ('Attrezzatura cucina', 'Attrezzatura cucina'), ('Altro', 'Altro'))", "(('Elettrico', 'Elettrico'), ('Idraulico', 'Idraulico'), ('Muratore', 'Muratore'), ('Climatizzazione', 'Climatizzazione'), ('Porta/Finestra', 'Porta / Finestra'), ('Attrezzatura cucina', 'Attrezzatura cucina'), ('Altro', 'Altro'))")
text = text.replace("{'Elettrico', 'Idraulico', 'Climatizzazione', 'Porta/Finestra', 'Attrezzatura cucina', 'Altro'}", "{'Elettrico', 'Idraulico', 'Muratore', 'Climatizzazione', 'Porta/Finestra', 'Attrezzatura cucina', 'Altro'}")
...
def whatsapp_public_base_url():
    ...
    if tunnel:
        return tunnel
    return (options.get('public_base_url') or '').strip().rstrip('/')

@admin_app.get('/settings/whatsapp', response_class=HTMLResponse)
def whatsapp_settings(message: str = ''):
    con = db(); contacts = con.execute('SELECT * FROM whatsapp_contacts ORDER BY category,name').fetchall(); con.close()
    rows = ''
    for contact in contacts:
        status = 'Attivo' if contact['active'] else 'Disattivato'; toggle_label = 'Disattiva' if contact['active'] else 'Attiva'
    ...
    if not rows: rows = '<p class="muted">Nessun tecnico configurato.</p>'
...
def whatsapp_contact_create(name: str = Form(...), phone: str = Form(...), category: str = Form('Tutte')):
    name = name.strip()
    if not name or len(name) > 100: raise HTTPException(400, 'Nome contatto non valido')
    if category not in WHATSAPP_CATEGORIES: raise HTTPException(400, 'Categoria non valida')
    phone = normalize_whatsapp_phone(phone)
    con = db(); con.execute('INSERT INTO whatsapp_contacts(name,phone,category,active,created_at) VALUES(?,?,?,?,?)', (name, phone, category, 1, now_iso())); con.commit(); con.close()
...
def whatsapp_contact_toggle(contact_id: int):
    con = db(); con.execute('UPDATE whatsapp_contacts SET active=CASE active WHEN 1 THEN 0 ELSE 1 END WHERE id=?', (contact_id,)); con.commit(); con.close()
...
def whatsapp_contact_delete(contact_id: int):
    con = db(); con.execute('DELETE FROM whatsapp_shares WHERE contact_id=?', (contact_id,)); con.execute('DELETE FROM whatsapp_contacts WHERE id=?', (contact_id,)); con.commit(); con.close()
    return RedirectResponse('../../whatsapp', status_code=303)
...
def whatsapp_share_ticket(ticket_id: int, contact_id: int):
    ...
    if not ticket or not contact:
        con.close(); raise HTTPException(404, 'Ticket o contatto non trovato')
    base = whatsapp_public_base_url()
    if not base:
        con.close(); raise HTTPException(409, 'Configura prima URL pubblico o tunnel pubblico nelle impostazioni dell’add-on.')
    token = whatsapp_ticket_token(ticket_id)
    link = f'{base}/w/{urllib.parse.quote(token, safe="")}'
    message = (f'🔧 Nuovo intervento Hausmeister Carellas\nTicket: {ticket["ticket_code"]}\nZona: {ticket["zone_name"]}\nCategoria: {ticket["category"]}\nPriorità: {ticket["priority"] or "Normale"}\nProblema: {ticket["description_original"][:700]}\n\n📋 Ticket completo e foto: {link}')
    con.execute('INSERT INTO whatsapp_shares(ticket_id,contact_id,sent_at) VALUES(?,?,?)', (ticket_id, contact_id, now_iso())); con.commit(); con.close()
...
'''
    if route_marker not in text: raise SystemExit('WhatsApp route marker not found')
...
def whatsapp_public_file(signed_token: str, file_id: int):
    ticket_id = whatsapp_ticket_id(signed_token)
    con = db(); row = con.execute('SELECT * FROM ticket_files WHERE id=? AND ticket_id=?', (file_id, ticket_id)).fetchone(); con.close()
