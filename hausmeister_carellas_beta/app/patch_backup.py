...
# Imports necessari per backup automatici e ripristino.
if 'import threading\n' not in text:
    text = text.replace('import time\n', 'import time\nimport threading\n', 1)

# Directory e funzioni backup.
helpers_marker = '\n\ninit_db()\n'
...
def backup_filename(kind: str = 'manual'):
    safe_kind = kind if kind in {'manual', 'daily', 'weekly', 'monthly', 'safety'} else 'manual'
    return f'hausmeister-{safe_kind}-{datetime.now().strftime("%Y%m%d-%H%M%S")}.zip'


def create_backup_file(kind: str = 'manual'):
    filename = backup_filename(kind)
...
def backup_files():
    files = [p for p in BACKUP_DIR.glob('hausmeister-*.zip') if p.is_file()]
    return sorted(files, key=lambda p: p.stat().st_mtime, reverse=True)


def prune_backups():
    limits = {'daily': 7, 'weekly': 4, 'monthly': 6, 'safety': 3}
...
def validate_backup_archive(archive: zipfile.ZipFile):
...
def restore_backup_file(target: Path):
    ...
    try:
        ...
        for current in UPLOAD_DIR.iterdir():
            if current.is_file():
                try:
                    current.unlink()
                except OSError:
                    pass
        for restored in temp_uploads.iterdir():
            if restored.is_file():
                restored.replace(UPLOAD_DIR / restored.name)
        prune_backups()
    finally:
        try:
            temp_db.unlink()
...
# Avvio scheduler quando parte l'add-on.
...
def start_backup_scheduler():
    ...
    thread = threading.Thread(target=backup_scheduler_loop, name='hausmeister-backup', daemon=True)
    thread.start()
'''
...
# Trasforma il vecchio pulsante Backup delle Impostazioni in accesso al gestore.
...
# Sostituisce la vecchia rotta di download con il gestore completo.
...
def backup_restore_existing(filename: str):
    ...
    return RedirectResponse('../../backup?message=' + urllib.parse.quote('Ripristino completato. Riavvia l’add-on per applicare completamente i dati ripristinati.'), status_code=303)
